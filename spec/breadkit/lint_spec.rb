...
RSpec.describe Breadkit::Lint::Engine do
  let(:examples) { File.expand_path("../../../breadkit/examples", __dir__) }

  it "leaves the valid examples without warnings or errors" do
    files = %w[01_led_button.bk.rb 02_555_blinker.bk.rb 03_arduino_blink.bk.rb 04_led_bar.bk.rb 05_sensor_demo.bk.rb].map { |name| File.join(examples, name) }
    results = described_class.new.run(files)
    expect(results.flat_map { |item| item[:offenses] }.map(&:severity)).to all(eq("info"))
  end
